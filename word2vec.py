...
flags.DEFINE_string("save_path", None, "Directory to write the model and "
                    "training summaries.")
flags.DEFINE_string("checkpoint", None, "Checkpoint to restore a model (either path or latest) "
                    "and enter either continue training or inference mode.")
...
class Word2Vec(object):
  ...
  def forward(self, examples, labels):
    ...
    with tf.name_scope('Embexp'):
        example_emb = tf.nn.embedding_lookup(emb, examples)

    with tf.name_scope('Emb_Labels'):
        # Weights for labels: [batch_size, emb_dim]
        true_w = tf.nn.embedding_lookup(sm_w_t, labels)
        # Biases for labels: [batch_size, 1]
        true_b = tf.nn.embedding_lookup(sm_b, labels)

    with tf.name_scope('Emb_Samples'):
    ...
    with tf.name_scope('TrueLogits'):
        true_logits = tf.reduce_sum(tf.multiply(example_emb, true_w), 1) + true_b
        #tf.summary.scalar('truelogits', true_logits)
