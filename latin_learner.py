...
if __name__ == '__main__':
    ...
    parser.add_argument('--datadir', type=str, default='small_library',
                        help="Specify the directory with the latin texts.")
    ...
    if FLAGS.mode == 'LSTM':
        ...
        num_steps = 64          # Number of sequence steps per batch (default 100)
        ...
        latin_text = data.load_character_data(FLAGS.datadir)
        if len(latin_text) > 6000000:
            # If there is enough data, we can reduce the size of test and validation set 
            split_factor = 0.99
        else:
            split_factor = 0.9
        integer_text, int2char_mapping, char2int_mapping, character_set_size = data.generate_character_dataset(latin_text)
        train_text, val_text, test_text = data.split_int_text(integer_text, split_factor)
        ...
        texts = data.load_tokenized_data(FLAGS.datadir)
